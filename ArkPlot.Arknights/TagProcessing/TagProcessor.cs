...
public partial class TagProcessor
{
    ...
    /// <summary>处理一行 FormattedTextEntry，返回 Markdown 文本。供适配器调用。</summary>
    ...
    {
        _notify.OnNoMatchTag(new LineNoMatchEventArgs(line, tag));
        return line;
    }
    ...
    private static string HandleEmptyValue(string newTag)
    {
        switch (newTag)
        {
            case "`音乐停止`":
