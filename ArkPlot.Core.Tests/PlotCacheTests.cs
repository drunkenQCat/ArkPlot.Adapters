...
public class PlotCacheTests : IDisposable
{
    ...
    public async Task SaveAsync_And_TryLoadAsync_Roundtrip()
    {
        ...
        await PlotCache.SaveAsync(plot, entries, PlotStatus.Parsed, _db);
        var result = await PlotCache.TryLoadAsync(1, "测试章节", _db);
    ...
    public async Task GetCachedTitlesAsync_ReturnsOnlyStatus2()
    {
        ...
        await PlotCache.SaveAsync(plot1, [new() { Index = 0, OriginalText = "x" }], PlotStatus.Parsed, _db);
        await PlotCache.SaveAsync(plot2, [new() { Index = 0, OriginalText = "y" }], PlotStatus.Downloaded, _db);

        var titles = await PlotCache.GetCachedTitlesAsync(1, _db);
    ...
    {
        var result = await PlotCache.TryLoadAsync(999, "不存在", _db);
        Assert.Null(result);
    }
    ...
    public async Task CleanupEmptyPlotsAsync_RemovesEmptyEntries()
    {
        ...
        var emptyEntries = new List<FormattedTextEntry>
        {
            new() { Index = 0, OriginalText = "" },
            new() { Index = 1, OriginalText = "  " },
        };
        await PlotCache.SaveAsync(plot, emptyEntries, PlotStatus.Parsed, _db);

        var cleaned = await PlotCache.CleanupEmptyPlotsAsync(1, _db);

        Assert.Equal(1, cleaned);
        var result = await PlotCache.TryLoadAsync(1, "空章节", _db);
        Assert.Null(result);
    }
    ...
    public async Task CleanupEmptyPlotsAsync_KeepsNonEmptyEntries()
    {
        ...
        var result = await PlotCache.TryLoadAsync(1, "有内容", _db);
        Assert.NotNull(result);
    }
    ...
    public async Task SaveAsync_Upsert_UpdatesExistingPlot()
    {
        ...
        await PlotCache.SaveAsync(plot, entries1, PlotStatus.Parsed, _db);
        ...
        await PlotCache.SaveAsync(plot, entries2, PlotStatus.Parsed, _db);

        var result = await PlotCache.TryLoadAsync(1, "更新测试", _db);
        Assert.NotNull(result);
        Assert.Equal(2, result.Value.Entries.Count);
    ...
    public async Task SaveAsync_Status1_NotReturnedByGetCachedTitles()
    {
        ...
        await PlotCache.SaveAsync(plot, [new() { Index = 0, OriginalText = "x" }], PlotStatus.Downloaded, _db);
    ...
    public async Task TryLoadAsync_EntriesAreOrderedByIndex()
    {
        ...
        var result = await PlotCache.TryLoadAsync(1, "排序测试", _db);
        Assert.NotNull(result);
