...
public class Plot
{
    ...
    /// <summary>
    /// 导航到解析后的 ScriptLine 列表（一对多）
    /// </summary>
    [Navigate(NavigateType.OneToMany, nameof(ScriptLine.PlotId))]
    public List<ScriptLine> Entries { get; set; } = [];
    ...
    /// <summary>
    /// 解析后的文本条目列表（运行时使用，不存库；入库后在对应游戏的表中）
    /// </summary>
    [SugarColumn(IsIgnore = true)]
    public List<ScriptLine> TextVariants { get; set; } = [];
